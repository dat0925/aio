...
## 9. 機能要件・競合比較(2026-06-23時点)

### 競合ツールの分類

市場のAIO/GEOチェッカーは大きく3タイプに分かれる。

**タイプ①:実際のAI回答に引用されているかを計測するモニタリング型**
Otterly.AI、ナイル「生成AIブランド言及チェッカー」など。ブランド名・質問文を入力するとChatGPT・Gemini・Perplexity等での言及の有無をリアルタイムで追跡する。継続監視が主目的。

**タイプ②:構造的にAIに引用されやすいかを診断する診断型**
aio-check.ai(990点満点・42項目)など。当ツールはこのタイプ。URLを入れるだけで自動解析する点が特徴。

**タイプ③:ユーザーのセルフ回答式チェックリスト型**
6カテゴリ30問・A〜Eグレードなど。自動解析ではなく自己申告に基づく。

### 当ツールの差別化ポイント

- URLを入れるだけで実際にページを取得・自動解析する(セルフ診断型より正確)
- 日本語・無料・施策まで出る(法人向け有料サービスは月額数万円〜)
- 診断結果をMarkdown形式でコピーしてAIに指示できる(AIとの連携を前提にした設計)

### 現時点で不足している機能(競合が持っているもの)

| 機能 | 優先度 | 備考 |
|---|---|---|
| 実際のAI回答をそのまま見せる(ChatGPT/Geminiでの引用確認) | 中 | 公式APIがないため実装難度が高い。Perplexity APIは取得可能だが代表性の問題あり |
| 診断履歴の保存・推移グラフ | 中 | 現状はセッション内のみ。DBを持てば実現できる |
| 複数URL一括診断 | 低 | 競合の多ページサイト向け。現状の個人ツール用途では不要 |
| キーワード別のAI引用状況追跡 | 低 | モニタリング型の機能。別カテゴリのツールになる |

### 今後の機能追加候補(SiteMeister連携を想定する場合)

当ツールを将来SiteMeister顧客向けに展開するとしたら、以下の順が筋がよい。

1. Safari/モバイル対応(Cloudflare Workers RouteでCORS問題を解消)
2. 診断履歴の保存(SupabaseのDBを追加し、月次推移を見られるようにする)
3. 競合URL比較(自社と近隣競合を並べてスコア比較)

SiteMeister連携の旧要件定義は `archive/requirements-sitemeister-concept.md` に保管済み。
