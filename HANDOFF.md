...
2. **Secret `ANTHROPIC_API_KEY` を登録する**
   `aio-check` → 「バインディング」タブ → Secretを追加 → 名前 `ANTHROPIC_API_KEY`、値はClaudeのAPIキー。
   (「バインディング」タブはコード編集画面ではなく、Workerのトップ画面の並びタブにある。コード編集画面からは「← aio-check」で一度戻る必要がある)
3. **動作確認**
   https://aio.taskra.jp を開き、URL欄に `https://flowra.taskra.jp/lp` を入れて「診断する」を押す。
   - 構造データだけ表示されAI所見が出ない場合 → Secret未設定、またはまだ反映されていない
